...
* **🌲 目录结构树生成**
    ...
    * 实时预览：支持界面即时预览前 50 行，并提供**一键复制完整结构**功能。
* **🏷️ 批量重命名 (ReNamer 极简版)**
    * 打字即预览：500ms 毫秒级无感防抖刷新，告别繁琐的“点击生成”按钮。
    * 智能序列引擎：支持自定义起步值、步长与**自动补零**（如 001, 002），强制按字母顺序排队处理。
    * 安全防呆机制：内置命名冲突与正则语法校验，保护工程文件安全。
* **🎨 全天候动态主题与高分屏优化**
    * 内置极简白、暗夜黑、护眼绿等五套配色，全局 1px 无边框 UI 设计。
    * 底层进行 DPI 感知适配，确保高分屏下文字清晰、界面不变形。
